import { SubmittedData } from "./hooks/step9";
import sampleData from "../data/application.json" assert { type: "json" }; // optional

const applicationData = SubmittedData.value || sampleData;
...
function loadDataForStep1() {
  ...
  if (
    applicationData.step1.industry &&
    industryMap[applicationData.step1.industry]
  ) {
      ...
      applicationData.step5.people.forEach((person, index) => {
        if (index < rows.length) {
          ...
          if (numberInput && person.number) {
            ...
            console.log(
              `Set auth person ${index + 1} number = ${person.number}`
            );
          }
        }
      });
...
function loadDataForStep3() {
  ...
  const accountTypes = [
    {
      data: "acc_cca",
      selector:
        'input[type="checkbox"]:has(+ strong:contains("Corporate Cheque Account"))',
    },
    {
      data: "acc_bia",
      selector:
        'input[type="checkbox"]:has(+ strong:contains("Business Interest Account"))',
    },
    {
      data: "acc_bca",
      selector:
        'input[type="checkbox"]:has(+ strong:contains("Business Everyday Account"))',
    },
    {
      data: "acc_bcm",
      selector:
        'input[type="checkbox"]:has(+ strong:contains("Business Cash Maximiser"))',
    },
    {
      data: "acc_td",
      selector: 'input[type="checkbox"]:has(+ strong:contains("Term Deposit"))',
    },
  ];
  ...
  // NAB Connect MEID
  const meidInput = document.querySelector(
    'input[placeholder*="MEID"], input[class*="meid"]'
  );
  ...
  const ibUsersTextarea = document.querySelector(
    'textarea[placeholder*="NAB ID"], textarea[class*="ib-users"]'
  );
